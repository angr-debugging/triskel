...
TEST(Attribute, Node) {
    ...
    ASSERT_EQ(test_attribute[n7], 42);

    test_attribute[n7] = 0;

    ASSERT_EQ(test_attribute[n7], 0);
}
...
TEST(Attribute, Edge) {
    ...
    ASSERT_EQ(test_attribute[e6_3], 42);

    test_attribute[e6_3] = 0;

    ASSERT_EQ(test_attribute[e6_3], 0);
}
...
TEST(Attribute, Resize) {
    ...
    ASSERT_EQ(test_attribute[n1], 42);

    fmt::print("Reading value for n2\n");
    ASSERT_EQ(test_attribute[n2], 42);

    fmt::print("Setting value for n4\n");
    test_attribute[n4] = 0;

    ASSERT_EQ(test_attribute[n4], 0);
}
